...
export function LauncherChat({
  ...
  /* Which face of the panel is showing. The conversation keeps running
     underneath while the list is up — nothing here tears the thread down, so
     going back and returning lands exactly where you were. */
  ...
  return (
    <ChatbotShell
      onClose={onClose}
      /* Back goes up to the conversation list, not out of the panel. Closing
         is the × alongside it; a left chevron that shuts the window is the
         wrong shape for the glyph, and it was doing exactly that — onBack
         and onClose were the same handler. */
      onBack={() => setView("history")}
      onSend={handleSend}
      accent={PURPLE}
      style={style}
      overlay={
        /* Slides in from the left rather than appearing. The direction is
           the point: back moved left, so the list arrives from the left, and
           choosing a conversation sends it back the way it came. It's kept
           mounted and translated off rather than unmounted, so the slide has
           something to animate and the list doesn't rebuild each time.

           Same curve and duration as the sibling implementation in
           ChatWindow, which is where this pattern already exists — two
           surfaces doing the same navigation shouldn't move differently. */
        <div
          /* Matches the shell's paper, not white. The overlay is what you see
             mid-slide, so a cool white sheet moving across a warm panel
             flashes the wrong colour on every transition. */
          className="absolute inset-0 flex flex-col bg-[#FEFCF8]"
          style={{
        ...
        >
          {/* No voice on this surface, so no voice tab and no tab bar with
              it — the list is the only thing this view does. */}
          <ChatHistory
            voice={false}
            /* The same purple the shell's focus ring and send button use, so
               the list belongs to this tenant rather than carrying the blue
               the component happened to ship with. */
            accent={PURPLE}
